let standardRanks = ['S1', 'S2', 'S3', 'G1', 'G2', 'G3', 'P1', 'P2', 'P3', 'D1', 'D2', 'D3', 'C1', 'C2', 'C3', 'GC1', 'GC2', 'GC3', 'SSL'];
...
const modeSelector = document.querySelector('#gamemode');
...
for (let i = 0; i < priceList.length; i++) {
	...
	plnCell.appendChild(document.createTextNode(priceList[counter][1].toFixed(2) + ' PLN'));
...
function calculate() {
	...
	let pricePercent = gamemodes[selectedMode] + paymentMethods[selectedPaymentMethod];
	...
	if (isPlaying && selectedMode != 'solo') {
	...
	if (actualRankID >= desiredRankID) {
		priceTextElement.innerHTML = 'Docelowa ranga musi być <strong class="highlight-y">wyższa</strong> niż obecna!';

	} else if ((selectedMode != 'solo') && (desiredRankID > 15)) {
		priceTextElement.innerHTML = 'Rangi <strong class="highlight-y">GC2 - SSL</strong> możliwe do wbicia tylko na trybie <strong class="highlight-y">1v1</strong>!';
	} else {
